...
if __name__ == "__main__":
    ...
    df_train["answer"]   = np.random.rand(nrows)
    df_valid = pd.DataFrame(np.random.rand(nrows, df_train.shape[1]), columns=df_train.columns.copy())
    df_valid["answer"]   = np.random.rand(nrows)

    # set manager
    manager = MLManager(df_train.columns[df_train.columns.str.contains("^col_")].tolist(), "answer", is_reg=True)
    # pre processing
    ...
    # set model
    manager.set_model(RandomForestRegressor, bootstrap=True, n_estimators=100, max_depth=None, n_jobs=1)
    # registry proc
    manager.proc_registry(dict_proc={
        ...
        "ans": [
            '"ProcAsType", np.float32, n_jobs=1',
            '"ProcToValues"',
            '"ProcReshape", (-1, )',
        ]
    })
