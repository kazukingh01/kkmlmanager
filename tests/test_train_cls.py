...
from kkgbdt.loss import CategoricalFocalLoss, Accuracy
# local package
...
if __name__ == "__main__":
    ...
    manager.cut_features_by_correlation(df_train, cutoff=None, dtype='float32', is_gpu=False, corr_type='spearman',   batch_size=2, min_n=100, sample_size=1000)
    manager.cut_features_by_correlation(df_train, cutoff=None, dtype='float32', is_gpu=False, corr_type='chatterjee', batch_size=2, min_n=100, sample_size=1000)
    list_proc = [
        "self.initialize()",
        "self.cut_features_by_variance(cutoff=0.995, ignore_nan=False)",
        "self.cut_features_by_variance(cutoff=0.995, ignore_nan=True )",
        "self.cut_features_by_adversarial_validation(cutoff=2.3, thre_count='mean')",
        "self.cut_features_by_randomtree_importance( cutoff=0.9)",
        "self.cut_features_by_correlation(cutoff=0.9, corr_type='chatterjee')",
        "self.cut_features_by_correlation(cutoff=0.9, corr_type='pearson')",
        "self.cut_features_by_correlation(cutoff=0.9, corr_type='spearman')",
    ]
    manager.cut_features_auto(list_proc=list_proc)
    ins1 = MLManager.from_dict(manager.to_dict()).cut_features_auto(list_proc=list_proc)
    ins2 = MLManager.from_json(manager.to_json()).cut_features_auto(list_proc=list_proc)
    ins3 = manager.copy().cut_features_auto(list_proc=list_proc)
    manager.save(dirpath="./tmp/", filename="tmp", is_remake=True, is_minimum=False, is_json=True, mode=1)
    ins4 = MLManager.load(filepath="./tmp/tmp.json", n_jobs=manager.n_jobs).cut_features_auto(list_proc=list_proc)
    manager.save(dirpath="./tmp/", filename="tmp", is_remake=False, is_minimum=False, is_json=False)
    ins5 = MLManager.load(filepath="./tmp/tmp.pickle", n_jobs=manager.n_jobs).cut_features_auto(list_proc=list_proc)
    ...
    # set model
    n_class = df_train[dataset.metadata.columns_target].unique().len()
    ...
    manager.fit(
        ...
        params_fit={"loss_func": "multi", "num_iterations": 20, "sample_weight": "balanced", "x_valid": "_validation_x", "y_valid": "_validation_y"}
    )
    ...
    manager.fit_cross_validation(
        ...
        params_fit_evaldict={
            "loss_func": CategoricalFocalLoss(n_class, gamma=0.5, dx=1e-5), "num_iterations": 20, "loss_func_eval": ["__copy__", Accuracy(top_k=2)],
